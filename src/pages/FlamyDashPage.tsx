...
export const FlamyDashPage: React.FC = () => {
  const navigate = useNavigate();
  const [isPlaying, setIsPlaying] = useState(false);
  const [isLiked, setIsLiked] = useState(false);
  const [isBookmarked, setIsBookmarked] = useState(false);

  // Get Flamy Dash game data
  const flamyDashGame = allGames.find(game => game.id === 'flamy-dash');

  // Get recommended games based on similar gameplay
  const getRecommendedGames = () => {
    if (!flamyDashGame) return [];
    
    return allGames
      .filter(g => {
        if (g.id === flamyDashGame.id) return false;
        
        // Prioritize similar categories and tags
        const sharedTags = g.tags.filter(tag => flamyDashGame.tags.includes(tag));
        const sameCategory = g.category === flamyDashGame.category;
  ...
  const gameplayHighlights = [
    "Lightning-fast dash mechanics that test your reflexes",
    "Challenging obstacle courses with increasing difficulty",
    "Stunning visual effects and smooth animation",
  ...
  useEffect(() => {
    ...
    if (flamyDashGame) {
      ...
      setIsLiked(likedGames.includes(flamyDashGame.id));
      setIsBookmarked(bookmarkedGames.includes(flamyDashGame.id));
    }
  }, [flamyDashGame]);
  ...
  const handleLike = () => {
    ...
    if (isLiked) {
      updatedLikes = likedGames.filter((id: string) => id !== flamyDashGame.id);
    } else {
      updatedLikes = [...likedGames, flamyDashGame.id];
    }
  ...
  const handleBookmark = () => {
    ...
    if (isBookmarked) {
      updatedBookmarks = bookmarkedGames.filter((id: string) => id !== flamyDashGame.id);
    } else {
      updatedBookmarks = [...bookmarkedGames, flamyDashGame.id];
    }
  ...
  const handleShare = async () => {
    const shareData = {
      title: `Flamy Dash - New Game Recommendation | QueensGame`,
      text: `Check out this amazing dash adventure game: Flamy Dash!`,
  ...
  return (
    <div className="min-h-screen relative">
      ...
      <SEOHead 
        game={flamyDashGame}
        title="Flamy Dash - New Game Recommendation | Fast-Paced Action Adventure"
        description="Experience the ultimate dash adventure in Flamy Dash! Perfect for reflex-testing players and fast-paced action lovers. Play this challenging new game free online now!"
        keywords="flamy dash, new game recommendation, dash adventure, reflex game, fast-paced action, obstacle course, arcade game, challenging gameplay, free online game"
      />
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
                <div>
                  ...
                  <p className="text-xl text-gray-300 mb-6 leading-relaxed">
                    Experience the ultimate dash adventure with challenging obstacles and stunning visual effects. 
                    Perfect for reflex-testing players who love fast-paced action!
                  </p>
                  ...
                  <div className="grid grid-cols-4 gap-4">
                    <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 text-center">
                      ...
                      <div className="text-white font-bold">{flamyDashGame.rating?.toFixed(1)}</div>
                    ...
                    <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 text-center">
                      ...
                      <div className="text-white font-bold">
                        {flamyDashGame.plays ? `${Math.floor(flamyDashGame.plays / 1000)}k` : '15k'}
                      </div>
                ...
                <div className="relative">
                  <img
                    src={flamyDashGame.thumbnail}
                    alt="Flamy Dash Game Screenshot"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
              ...
              <div className="lg:col-span-2 space-y-8">
                  ...
                  <div className="grid gap-4">
                    {gameplayHighlights.map((highlight, index) => (
                      <div key={index} className="flex items-start space-x-3">
                        <div className="bg-purple-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-sm font-bold flex-shrink-0 mt-1">
                          {index + 1}
                        </div>
                ...
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {playerTypes.map((type, index) => (
                      <div key={index} className="bg-gradient-to-r from-green-500/10 to-blue-500/10 rounded-lg p-4 border border-green-400/20">
                ...
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
                  ...
                  <ol className="space-y-3">
                    {flamyDashGame.instructions?.map((instruction, index) => (
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="text-white">{flamyDashGame.author}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-300">Category:</span>
                      <span className="text-white">{flamyDashGame.category}</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-white">{flamyDashGame.language}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-300">Size:</span>
                      <span className="text-white">{flamyDashGame.width}x{flamyDashGame.height}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
                  ...
                  <button
                    onClick={() => navigate(`/?category=${encodeURIComponent(flamyDashGame.category)}`)}
                    className="w-full mt-4 text-purple-400 hover:text-purple-300 text-sm font-medium transition-colors"
                  >
                    View All {flamyDashGame.category} Games →
                  </button>
                </div>
              </div>
            </div>
